...
## 📄 Conceptos Clave: El Manifiesto "Application"

ArgoCD utiliza un recurso personalizado (CRD) llamado `Application` para definir qué desplegar. Ejemplo básico:

```yaml
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: guestbook # Nombre en ArgoCD
  namespace: argocd # Donde vive ArgoCD
spec:
  project: default
  source:
    repoURL: https://github.com/argoproj/argocd-example-apps
    path: guestbook # Carpeta dentro del repo
    targetRevision: HEAD
  destination:
    server: https://kubernetes.default.svc
    namespace: guestbook # Donde se desplegará la app
  syncPolicy:
    syncOptions:
